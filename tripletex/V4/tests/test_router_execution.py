...
class RouterExecutionTests(unittest.TestCase):
    ...
    def test_attachment_import_flow_skips_restricted_incoming_invoice_read_when_not_needed(self) -> None:
        transport = RecordingTransport(
            {
                ("GET", "/supplier"): {"values": []},
                ("POST", "/supplier"): {"value": {"id": 44, "name": "ACME Supplier"}},
                ("POST", "/ledger/voucher/importDocument"): {"value": {"id": 321}},
            }
        )
        context = ExecutionContext(
        ...
        bridge = LLMBridgeDocument.model_validate(
            {
                ...
                "flatBridge": {
                    "flowArguments": {
                        "supplier_invoice.import_from_attachment": {
                            ...
                            "description": "Import supplier invoice",
                        }
                    }
                },
                "executionPlan": {
                ...
                "validation": {"isExecutable": True, "blockingIssues": [], "warnings": []},
                "completion": {"completionSignals": ["Supplier invoice imported"]},
            }
        )

        result = router.execute(bridge, context)
        self.assertEqual(result.traces[0].outputs["imported"], {"value": {"id": 321}})
        self.assertNotIn("incomingInvoice", result.traces[0].outputs)
        self.assertEqual(
            [call["path"] for call in transport.calls],
            ["/supplier", "/supplier", "/ledger/voucher/importDocument"],
        )

    def test_router_resolves_step_output_bindings_between_flow_steps(self) -> None:
        def customer_search(_params: dict[str, Any], _body: dict[str, Any]) -> Any:
            return {"values": []}

        def customer_create(_params: dict[str, Any], body: dict[str, Any]) -> Any:
            self.assertEqual(body["name"], "ACME AS")
            return {"value": {"id": 17, "name": "ACME AS"}}
                ...
                "flatBridge": {
                    "flowArguments": {
                        ...
                        "project.create_for_customer": {
                            "name": "ACME Build",
                            "customer": {"$fromStep": "flow_1", "path": "value.id"},
                        },
                    }
                },
                "executionPlan": {
        ...
        result = router.execute(bridge, self._context())
        self.assertEqual(result.traces[1].outputs, {"value": {"id": 55, "name": "ACME Build"}})
        self.assertEqual([call["path"] for call in transport.calls], ["/customer", "/customer", "/project"])
