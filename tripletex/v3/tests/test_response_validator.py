...
class ResponseValidatorTests(unittest.TestCase):
    ...
    def test_string_top_level_sections_are_normalized_instead_of_crashing(self) -> None:
        bridge = self.validator.validate(
            {
                "contractVersion": "tripletex.llm_bridge.v1",
                "language": "nb",
                "understanding": "register invoice payment",
                "validation": {"isExecutable": False},
                "__tripletex_defaults": {
                    "prompt": "Register payment for invoice 1001",
                    "currentDate": "2026-03-22",
                    "timezone": "Europe/Oslo",
                    "requestId": "req-str-sections",
                    "attachmentCount": 0,
                    "attachments": [],
                },
            }
        ...
        self.assertEqual(bridge.language.promptOriginal, "Register payment for invoice 1001")
        self.assertEqual(bridge.language.promptCanonical, "Register payment for invoice 1001")
        self.assertEqual(bridge.understanding.objective, "Register payment for invoice 1001")
        self.assertFalse(bridge.validation.isExecutable)
    ...
    def test_canonicalizes_flow_prefixed_argument_names_back_to_legal_flow_inputs(self) -> None:
        bridge = self.validator.validate(
            {
                "contractVersion": "tripletex.llm_bridge.v1",
                "language": {"promptOriginal": "pay invoice", "promptCanonical": "pay invoice"},
                ...
                "flatBridge": {
                    "flowArguments": {
                        "invoice.register_payment": {
                            "invoice_register_payment_invoice_selector": {"invoice_number": "1001"},
                            "invoice_register_payment_payment_spec": {
                                "payment_date": "2026-03-21",
                                "payment_type_ref": {"description": "Bank payment"},
                                "paid_amount": 1250.0,
                            },
    ...
    def test_nests_flat_flow_arguments_for_single_selected_flow(self) -> None:
        bridge = self.validator.validate(
            {
                "contractVersion": "tripletex.llm_bridge.v1",
                "language": {"promptOriginal": "create invoice", "promptCanonical": "create invoice"},
                "understanding": {"objective": "create invoice"},
                "flatBridge": {
                    "flowArguments": {
                        ...
                        "line_items": [
                            {
                                ...
                                "count": 1,
                                "unit_price_ex_vat": 6400,
                                "currency_ref": {"code": "NOK"},
                            }
                        ],
        ...
        self.assertIn("invoice.order_first", bridge.flatBridge.flowArguments)
        self.assertEqual(bridge.flatBridge.flowArguments["invoice.order_first"]["order_date"], "2026-03-22")
        self.assertEqual(bridge.flatBridge.flowArguments["invoice.order_first"]["customer"], {"name": "ACME AS"})

    def test_canonicalizes_raw_payload_aliases_inside_wrapper_payload_family(self) -> None:
        bridge = self.validator.validate(
            {
                "contractVersion": "tripletex.llm_bridge.v1",
                "language": {"promptOriginal": "create voucher", "promptCanonical": "create voucher"},
                ...
                "executionPlan": {
                    "selectedFlows": [
                        {
                            "stepId": "step_1",
                            "flowName": "voucher.manual_adjustment",
                            "flowType": "business_flow",
                        }
                    ]
                },
        ...
        posting = bridge.flatBridge.flowArguments["voucher.manual_adjustment"]["postings"][0]
        self.assertEqual(posting["account_ref"], 1200)
        self.assertEqual(posting["supplier_ref"], {"name": "ACME AS"})
