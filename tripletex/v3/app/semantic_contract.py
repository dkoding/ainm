...
def canonicalize_payload_value(family: str | None, value: Any) -> Any:
    ...
    raw_alias_lookup = {
        normalize_key(raw_name): field_name
        for field_name, raw_name in meta.get("rawFieldMap", {}).items()
    }
    ...
    for key, item in value.items():
        ...
        canonical = allowed_lookup.get(normalized_key) or raw_alias_lookup.get(normalized_key) or key
