...
class ResponseValidator:
    ...
    def _normalize_known_arguments(self, data: dict[str, Any]) -> None:
        ...
        if not isinstance(flat_bridge, dict):
            return
        self._normalize_argument_containers(data)
    ...
    def _normalize_flow_payload(self, flow_name: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        normalized = self._canonicalize_target_inputs(payload, input_names(meta["inputs"]), target_name=flow_name)
    ...
    def _normalize_command_payload(self, command_name: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        normalized = self._canonicalize_target_inputs(payload, self._legal_command_inputs(meta), target_name=command_name)
    ...
    def _normalize_argument_containers(self, data: dict[str, Any]) -> None:
        flat_bridge = data.get("flatBridge")
        execution_plan = data.get("executionPlan")
        if not isinstance(flat_bridge, dict) or not isinstance(execution_plan, dict):
            return

        selected_flows = [
            step.get("flowName") or step.get("name") or ""
            for step in execution_plan.get("selectedFlows", [])
            if isinstance(step, dict) and (step.get("flowName") or step.get("name"))
        ]
        flow_legal_inputs = {
            flow_name: set(input_names(self.wrapper_catalog.get_flow(flow_name)["inputs"]))
            for flow_name in selected_flows
            if self.wrapper_catalog.has_flow(flow_name)
        }
        flow_arguments = flat_bridge.get("flowArguments")
        if isinstance(flow_arguments, dict):
            flat_bridge["flowArguments"] = self._normalize_argument_map(
                argument_map=flow_arguments,
                target_names=selected_flows,
                legal_inputs_by_target=flow_legal_inputs,
            )

        selected_commands = [
            step.get("commandName") or step.get("command") or step.get("operationId") or ""
            for step in [*execution_plan.get("selectedCommands", []), *execution_plan.get("fallbackRawCommands", [])]
            if isinstance(step, dict) and (step.get("commandName") or step.get("command") or step.get("operationId"))
        ]
        command_legal_inputs: dict[str, set[str]] = {}
        for command_name in selected_commands:
            if self.wrapper_catalog.has_command(command_name):
                command_legal_inputs[command_name] = set(self._legal_command_inputs(self.wrapper_catalog.get_command(command_name)))
            elif self.raw_catalog.has(command_name):
                command_legal_inputs[command_name] = set(self._legal_raw_inputs(self.raw_catalog.get(command_name)))
        command_arguments = flat_bridge.get("commandArguments")
        if isinstance(command_arguments, dict):
            flat_bridge["commandArguments"] = self._normalize_argument_map(
                argument_map=command_arguments,
                target_names=selected_commands,
                legal_inputs_by_target=command_legal_inputs,
            )
    ...
    def _normalize_argument_map(
        ...
        *,
        ...
        legal_inputs_by_target: dict[str, set[str]],
    ) -> dict[str, Any]:
        if not target_names:
            return argument_map
        ...
        for key, value in argument_map.items():
            if key in target_name_set and isinstance(value, dict):
                normalized[key] = dict(value)
            else:
                pending[key] = value
        if not pending:
            return normalized or argument_map
        if len(target_names) == 1 and not normalized:
            return {target_names[0]: dict(pending)}
        unresolved: dict[str, Any] = {}
        for key, value in pending.items():
            normalized_key = normalize_key(clean_contract_name(str(key)))
            candidate_targets = [
                target_name
                for target_name in target_names
                if normalized_key
                in {
                    normalize_key(clean_contract_name(input_name))
                    for input_name in legal_inputs_by_target.get(target_name, set())
                }
            ]
            if len(candidate_targets) == 1:
                normalized.setdefault(candidate_targets[0], {})
                normalized[candidate_targets[0]][key] = value
                continue
    ...
    def _canonicalize_named_inputs(self, payload: dict[str, Any], legal_inputs: list[str]) -> dict[str, Any]:
        canonical_lookup = {
            normalize_key(clean_contract_name(name)): clean_contract_name(name)
            for name in legal_inputs
            if name
        }
        normalized: dict[str, Any] = {}
        for key, value in payload.items():
            cleaned_key = clean_contract_name(str(key))
            canonical_key = canonical_lookup.get(normalize_key(cleaned_key), cleaned_key)
            normalized[canonical_key] = value
        return normalized

    def _canonicalize_target_inputs(
        self,
        ...
        *,
        ...
        target_norm = normalize_key(clean_contract_name(target_name))
        normalized: dict[str, Any] = {}
        for key, value in payload.items():
            cleaned_key = clean_contract_name(str(key))
            normalized_key = normalize_key(cleaned_key)
            canonical_key = canonical_lookup.get(normalized_key)
            if canonical_key is None and target_norm:
                if normalized_key.startswith(target_norm):
                    canonical_key = canonical_lookup.get(normalized_key[len(target_norm) :])
            if canonical_key is None:
                suffix_matches = [
                    candidate
                    for candidate_key, candidate in canonical_lookup.items()
                    if normalized_key.endswith(candidate_key)
                ]
                if len(suffix_matches) == 1:
                    canonical_key = suffix_matches[0]
            if canonical_key is None:
