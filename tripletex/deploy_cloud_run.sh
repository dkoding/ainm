...
ENV_SPEC="^##^GOOGLE_CLOUD_PROJECT=${GOOGLE_CLOUD_PROJECT}##GOOGLE_CLOUD_LOCATION=${GOOGLE_CLOUD_LOCATION}##GEMINI_MODEL=${GEMINI_MODEL}##LOG_LEVEL=${LOG_LEVEL}##TRIPLETEX_API_KEY=${API_KEY}##TRIPLETEX_REQUEST_TIMEOUT=${REQUEST_TIMEOUT}##TRIPLETEX_WHOAMI_FIELDS=${WHO_AM_I_FIELDS}##TRIPLETEX_TIMEZONE=${TIMEZONE}"
...
start_log_tail() {
  ...
  nohup bash "${ROOT_DIR}/scripts/cloud_run_logs.sh" tail >> "${LOG_FILE}" 2>&1 &
  local pid=$!
