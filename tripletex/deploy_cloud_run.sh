...
ENABLE_LLM_STEP_PLANNING="${TRIPLETEX_ENABLE_LLM_STEP_PLANNING:-false}"
API_KEY="${TRIPLETEX_API_KEY:-}"
PYTHON_BIN="${TRIPLETEX_PYTHON_BIN:-}"

if [[ -z "${PYTHON_BIN}" ]]; then
  if [[ -x "/tmp/tripletex-venv/bin/python" ]]; then
    PYTHON_BIN="/tmp/tripletex-venv/bin/python"
  else
    PYTHON_BIN="python3"
  fi
fi
...
ENV_VARS=(
  ...
  "TRIPLETEX_ENABLE_LLM_STEP_PLANNING=${ENABLE_LLM_STEP_PLANNING}"
)

echo "Deploying ${SERVICE_NAME} to project ${GOOGLE_CLOUD_PROJECT} in ${REGION}"
echo "Running release-gate verification with ${PYTHON_BIN}"
"${PYTHON_BIN}" "${ROOT_DIR}/scripts/verify_release_gate.py"
