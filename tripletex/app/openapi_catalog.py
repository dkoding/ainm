...
class OpenApiCatalog:
    ...
    def operation_summary(self, operation_id: str) -> str:
        return str(self.operations[operation_id]["operation"].get("summary") or "").strip()

    def operation_description(self, operation_id: str) -> str:
        return str(self.operations[operation_id]["operation"].get("description") or "").strip()

    def capability_profile(self, operation_id: str) -> dict[str, Any]:
        summary = self.operation_summary(operation_id)
        description = self.operation_description(operation_id)
        combined = " ".join(part for part in (summary, description) if part).lower()
        is_restricted = any(marker in combined for marker in RESTRICTED_MARKERS)
        return {
            "summary": summary,
            "description": description,
            "isBeta": "[beta]" in summary.lower() or "[beta]" in description.lower(),
            "isDeprecated": "[deprecated]" in summary.lower() or "[deprecated]" in description.lower(),
            "isRestricted": is_restricted,
            "isPilotOnly": "pilot" in combined,
            "isPackageLimited": "package" in combined or "only available for" in combined or "only supported in" in combined,
        }
